#ifndef MODBUS_MAX_485
#define MODBUS_MAX_485
...
namespace ModbusMax485
{
	...
	class ModbusMax485slave
	{
		...
		uint16_t m_de_re_pin;  // driver enable - receiver enable pin del integrado MAX485
		uint8_t m_SlaveID;
		...
		void sendFrame(uint8_t *pFrame, uint8_t frameLength)
		{
			*(uint16_t*)&pFrame[frameLength] = crc16(pFrame, frameLength);
			digitalWrite(m_de_re_pin, HIGH); // integrado MAX485 en modo Tx
			m_pHardwareSerial->write(pFrame, frameLength + 2);
			m_pHardwareSerial->flush();
			digitalWrite(m_de_re_pin, LOW); // integrado MAX485 en modo Rx
		}
	...
	public:
		...
		//
		//
		void begin(uint32_t baud, HardwareSerial *pHardwareSerial = &Serial, uint16_t de_re_pin = 4, uint8_t slaveId = 1)
		{
			//Clear all registers
			...
			m_BaudRate = baud;
			m_de_re_pin = de_re_pin;
			m_SlaveID = slaveId;

			pinMode(m_de_re_pin, OUTPUT);
